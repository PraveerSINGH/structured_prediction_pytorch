...
data_loader_test  = dataloaders.remsenDataLoader(dataset=dataset_test,  opt=data_test_opt,  is_eval_mode=True) 

dataset_train = getattr(dataloaders,data_train_opt['dataset'])(data_train_opt['split']) 
data_loader_train = dataloaders.remsenDataLoader(dataset=dataset_train, opt=data_train_opt, is_eval_mode=False)   
