...
#                pdb.set_trace()
        ...
        def trainIterator(self, num_iters):
            ...
            for X, Ygt, Yest in zip(X_chunks, Ygt_chunks, Yest_chunks):
                ...
                var_Yest_init   = torch.autograd.Variable(X.narrow(1,3,1))
		var_reg_loss[0] = criterion_iter(var_Yest_init, var_Ygt)
                ...
                if num_iters > 1:
                    var_X             = torch.autograd.Variable(X.narrow(1,0,3))
                    ...
                    if criterion_eng_type == 'BCEDetAuxLoss':
                        ...
                        var_eng_loss[0] = det_lambda * criterion_eng(var_detEval)
                    var_eng_loss[0].backward(retain_variables=True) 
                    hook.remove()                    
