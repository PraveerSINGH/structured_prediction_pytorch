...
parser.add_argument('--exp',  default='stereo_IterGradDetReplaceV1',  help='config file with parameters of the experiment')
...
exp_config_file = os.path.join('.','exp_defs','stereoMatching', args_opt.exp+'.py')
...
if not isinstance(data_test_opt,(list, tuple)):
    data_test_opt = [data_test_opt,]
    
data_loader_test = []
for dtest_opt in data_test_opt:
    dtest_opt['num_workers']  = 1
    dataset_test = dataloaders.stereoDataset(dataset=dtest_opt['dataset'], split=dtest_opt['split'], root='./datasets') 
    data_loader_test.append(dataloaders.stereoDataLoader(dataset=dataset_test,  opt=dtest_opt,  is_eval_mode=True))
     
data_train_opt['num_workers'] = args_opt.num_workers
dataset_train = dataloaders.stereoDataset(dataset=data_train_opt['dataset'], split=data_train_opt['split'], root='./datasets' ) 
data_loader_train = dataloaders.stereoDataLoader(dataset=dataset_train, opt=data_train_opt, is_eval_mode=False)   
...
eval_stats = algorithm.evaluate(data_loader_test) # evaluate the algorithm
print("Evaluation results:")
for key, estats in eval_stats.items():
    print("==> Dataset %s:" % key)
    print(estats)